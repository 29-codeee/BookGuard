...
async function transition(
  ...
  await ctx.tx.query(
    ...
    [newId('evt'), bookingId, from, to, reason, evidence ? JSON.stringify(evidence) : null, operator]
  );
...
export async function holdInventory(req: HoldRequest): Promise<{ hold: HoldResult; response?: unknown }> {
  ...
  return runTx(async ctx => {
    ...
    const invRes = await tx.query<{
      ...
      `SELECT id, code, resource_type, price, available_quantity FROM inventory WHERE id = $1 FOR UPDATE`,
      [req.inventoryId]
    );
    ...
    await moveInventory(ctx, req.inventoryId, 'available', 'held', quantity);
...
export async function expireHold(holdId: string): Promise<boolean> {
  ...
  return runTx(async ctx => {
    ...
    if (!hold || hold.status !== 'ACTIVE') return false; // already confirmed/expired/released
    if (booking.status !== 'HELD') return false; // e.g. RECONCILING keeps its seat held
    if (!hold.is_expired) return false; // Redis fired early; DB clock is authoritative
    if (booking.claim_live) return false; // confirm in flight; it decides the outcome
...
export async function confirmBooking(req: ConfirmRequest): Promise<ConfirmResult> {
  ...
  try {
    ...
    if (compensateProviderRef) {
      console.warn(`[BookingEngine] Compensating orphan provider reservation ${compensateProviderRef}`);
      await providerAdapter.cancel(compensateProviderRef).catch(err => console.error(err));
    }
  }
}
